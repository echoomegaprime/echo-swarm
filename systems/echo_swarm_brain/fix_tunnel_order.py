#!/usr/bin/env python3
"""Move an environment-configured ingress rule before a wildcard/catch-all."""
...
def vault_get(service: str) -> str:
    body = {
        ...
        "context": {"bypass_reason": "swarm tunnel order fix"},
    }
...
def main() -> int:
    ...
    swarm_rule = {"hostname": HOSTNAME, "service": SERVICE}
    ...
    ingress.insert(wildcard_idx, swarm_rule)

    put_config(global_key, ingress)
    print(f"reordered hostname={HOSTNAME} before_wildcard_idx={wildcard_idx}")
    return 0
