...
def test_service_unit_is_profile_bound_and_state_isolated() -> None:
    ...
    assert (
        "Environment=MAXIMALIST_IDEMPOTENCY_FILE="
        "/var/lib/echo/maximalist-reconstructed-v05/idempotency.v1.json"
    ) in unit
    ...
    assert "Environment=MAXIMALIST_REQUEST_TIMEOUT_SECONDS=900" in unit
...
def test_live_smoke_is_bound_to_the_reconstructed_live_contract() -> None:
    ...
    assert 'EXPECTED_WORKER_VERSION = "0.2.5"' in smoke
